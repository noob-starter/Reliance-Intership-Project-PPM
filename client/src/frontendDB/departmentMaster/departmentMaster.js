...
const AllUsers = () => {
  ...
  const columns = [
    { title: "Department Id", field: "departmentId", dataKey: "deptId" },
    { title: "Department Name", field: "departmentName", dataKey: "name" },
  ...
  const downloadPdf = () => {
    swal({
      ...
      if (willDownload) {
        ...
        doc.setFontSize(40);
        doc.text(20, 20, "Department Details");
        doc.autoTable({
          theme: "grid",
          columns: columns,
          body: users,
        ...
        doc.save("DepartmentDetails.pdf");
        swal(" Downloaded Successfully !!", {
          icon: "success",
        });
      } else {
  ...
  const changeInSearch = () => {
    const getNewSearched = users.filter((user) => {
      if (searchTerm === "") {
        ...
        return user;
      } else if (user.deptId.toLowerCase().includes(searchTerm.toLowerCase())) {
        return user;
      } else {
        return null;
      }
  ...
  return (
    <div className="myContainer">
      <Navbar />
      <Link to="/myservices/deptM/add">
        <button className="myBtn">Add Dept +</button>{" "}
      </Link>
        ...
        <tbody>
          {perPage.map((user) => (
            <tr key={user._id}>
              ...
              <td>
                <Link to={`/myservices/deptM/edit/${user._id}`}>
