...
const AllUsers = () => {
  ...
  const downloadExcel = () => {
    swal({
      ...
      if (willDownload) {
        ...
        XLSX.writeFile(workBook, "GradeData.XLSX");
  ...
  const columns = [
    { title: "Grade Id", field: "gradeId", dataKey: "gradeId" },
    { title: "Grade Name", field: "gradeName", dataKey: "gradeName" },
  ...
  const changeInSearch = () => {
    const getNewSearched = users.filter((user) => {
      if (searchTerm === "") {
        ...
        user.gradeName.toLowerCase().includes(searchTerm.toLowerCase())
      ) {
        return user;
      } else if (
        user.gradeId.toLowerCase().includes(searchTerm.toLowerCase())
      ) {
        return user;
      } else {
        return null;
  ...
  return (
    <div className="myContainer">
      <Navbar />
      <Link to="/myservices/gradeM/add">
        <button className="myBtn">Add Grade +</button>{" "}
      </Link>
        ...
        <tbody>
          {perPage.map((user) => (
            <tr key={user._id}>
              ...
              <td>{user.desc}</td>
              <td>
                <Link to={`/myservices/gradeM/edit/${user._id}`}>
