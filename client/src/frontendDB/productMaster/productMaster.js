...
import { getUsers, deleteUser } from "./fetchProduct";
...
const AllUsers = () => {
  ...
  return (
    <div className="myContainer">
      <Navbar />
      <Link to="/myproducts/productM/add">
        <button className="myBtn">Add Product +</button>{" "}
      </Link>
        ...
        <tbody>
          {perPage.map((user) => (
            <tr key={user._id}>
              ...
              <td>{user.productRemarks}</td>
              <td>{user.maxRolls}</td>
              <td>
                <Link to={`/myproducts/productM/edit/${user._id}`}>
