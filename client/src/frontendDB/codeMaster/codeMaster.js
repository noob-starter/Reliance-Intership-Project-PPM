...
const AllUsers = () => {
  ...
  const getAllUsers = async () => {
    ...
    var codeData = response.data.users;
    setUsers(codeData);
    setPerPage(codeData.slice(0, 5));
  };
  ...
  const downloadExcel = () => {
    swal({
      ...
      if (willDownload) {
        ...
        XLSX.writeFile(workBook, "CodeData.XLSX");
  ...
  const columns = [
    { title: "Code Id", field: "codeId", dataKey: "codeId" },
  ...
  const changeInSearch = () => {
    const getNewSearched = users.filter((user) => {
      if (searchTerm === "") {
        return user;
      } else if (user.codeId.toLowerCase().includes(searchTerm.toLowerCase())) {
        return user;
      } else if (user.desc.toLowerCase().includes(searchTerm.toLowerCase())) {
        return user;
      } else {
        return null;
      }
  ...
  return (
    <div className="myContainer">
      <Navbar />
      <Link to="/myservices/codeM/add">
        <button className="myBtn">Add Code +</button>{" "}
      </Link>
        ...
        <tbody>
          {perPage.map((user) => (
            <tr key={user._id}>
              ...
              <td>{user.typeId}</td>
              <td>
                <Link to={`/myservices/codeM/edit/${user._id}`}>
